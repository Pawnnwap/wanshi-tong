...
class MacroDataModule(Module):
    ...
    prompt_zh = f"""
你是一位宏观经济数据分析师。请获取截至2026年6月5日-6月6日最新的宏观经济数据。

{_MACRO_TOOLS_ZH}

## 日期红线（必须遵守）
- 数据发布日期早于2026年6月5日的，必须在行末标注"（旧数据）"
- 禁止虚构数据发布日期
- 优先选取6月5日或6月6日有更新的数据
...
当前日期：2026年6月6日"""
    ...
    prompt_en = f"""
You are a macroeconomic data analyst. Get the latest macroeconomic data as of June 5-6, 2026.

{_MACRO_TOOLS_EN}

## Date Red Lines (MUST follow)
- Data published before June 5, 2026 MUST be marked "(old data)" at end of line
- DO NOT fabricate data publication dates
- Prioritize data updated on June 5 or June 6
